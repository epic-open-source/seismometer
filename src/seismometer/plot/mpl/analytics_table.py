...
class PerformanceMetrics:
    """
    A class to calculate and provide overall performance statistics and threshold-specific statistics across a list of models (scores) and targets.
    """
    ...
    def __init__(self, df : Optional[pd.DataFrame] = None, score_columns : Optional[List[str]] = None, target_columns : Optional[List[str]] = None, *, metric : str, 
                 metric_values : List[float] =[0.7, 0.8], title : str = "Model Performance Statistics", top_level : str = "Score", table_config : AnalyticsTableConfig, 
        ...
        statistics_data: Optional[pd.DataFrame]
            Additional performance metrics statistics, will be joined with the statistics data generated by the code, by default None.
        ...
        if self.df is not None:
            if not self.score_columns or not self.target_columns:
                raise ValueError("When df is provided, both 'score_columns' and 'target_columns' need to be provided as well.")
        ...
        self.spanner_colors = AnalyticsTableConfig.spanner_colors
        self.columns_show_percentages = AnalyticsTableConfig.columns_show_percentages
        self.columns_show_bar = AnalyticsTableConfig.columns_show_bar

        self.color_bar_style = ColorBarStyle(AnalyticsTableConfig.color_bar_style).value
        self.opacity = AnalyticsTableConfig.opacity
        self.style = TableStyle(AnalyticsTableConfig.style).value
        self.percentages_decimals = AnalyticsTableConfig.percentages_decimals
        self.alternating_row_colors = AnalyticsTableConfig.alternating_row_colors
        self.data_bar_stroke_width = AnalyticsTableConfig.data_bar_stroke_width
        ...
        # if polars package is not installed, overwrite is_na function in great_tables package to treat Agnostic as pandas dataframe.
        try:
            import polars as pl
        except:
            from great_tables._tbl_data import is_na, Agnostic, PdDataFrame
    ...
    def metric(self, value):
        try:
            ...
            raise ValueError(f"Invalid metric name: {value}. The metric needs to be one of: {[member.value for member in Metric]}")
    ...
    def top_level(self, value):
        try:
            ...
            raise ValueError(f"Invalid top_level name: {value}. The top_level needs to be one of: {[member.value for member in TopLevel]}")
    ...
    def generate_initial_table(self, data):
        ...
        gt = (gt.tab_stub(rowname_col=self._get_second_level[self.top_level], groupname_col=self.top_level)
                .fmt_number(columns = [col for col in data.columns if is_numeric_dtype(data[col].dtype) and not is_integer_dtype(data[col].dtype)], 
    ...
    def generate_color_bar(self, gt, columns):
        ...
        for data_col, col in itertools.product(columns,self.columns_show_bar):
            if data_col.endswith(f"_{col}_bar") or data_col == f"{col}_bar":
                gt = gt.fmt_nanoplot(f"{data_col}", plot_type="bar",
    ...
    def add_coloring_parity(self, gt, columns = None, even_color="#F2F2F2", odd_color="white"):
        ...
        """
        gt = (gt.tab_style(style=style.fill(color=even_color),
                    locations=loc.body(columns=columns, rows = [row for row in range(self.num_of_rows) if (row % self.rows_group_length) % 2 == 0]))
                .tab_style(style=style.fill(color=odd_color),
                    locations=loc.body(columns=columns, rows = [row for row in range(self.num_of_rows) if (row % self.rows_group_length) % 2 == 1])))
        return gt
    
    def group_columns_by_metric_value(self, gt, columns, value):
        gt = (gt.tab_spanner(label=f"{self.metric}={value}", columns=columns)
                .cols_label(**{col:"_".join(col.split('_')[1:]) for col in columns})
                .tab_style(style=style.borders(sides=["left"], weight='2px', color="black"),
                            locations=loc.body(columns=[columns[0]]))
                .tab_style(style=style.borders(sides=["right"], weight='2px', color="black"),
    ...
    def color_group_of_columns(self, gt, columns):
        if not self.spanner_colors:
            ...
            gt = self.add_coloring_parity(gt, columns, even_color=self.spanner_colors[self.spanner_color_index], 
                                        odd_color=lighten_color(self.spanner_colors[self.spanner_color_index]))
        else:
            gt = gt.tab_style(style=style.fill(color=self.spanner_colors[self.spanner_color_index]),
    ...
    def analytics_table(self):
        ...
        # Light gray/white alternating pattern needs to be corrected only if there are even many rows in each row-group.
        ...
        if self.color_bar_style == 1:
            ...
            for col in data.columns:
                if col in self.columns_show_bar:
                    data[col] = data[col].apply(lambda x: create_bar(x, max_width=75, height=20, color=self.columns_show_bar[col]))
                elif any(col.endswith(f"_{val}") for val in self.columns_show_bar):
                    data[col] = data[col].apply(lambda x: create_bar(x, max_width=75, height=20, color=self.columns_show_bar[col.split("_")[-1]]))
        return data
    ...
    def _generate_table_data(self):
        ...
        if self.df is not None:
            rows_list = []
            product = itertools.product(self.score_columns,self.target_columns) if self.top_level == "Score" else itertools.product(self.target_columns, self.score_columns)
            for first, second in product:
                ...
                current_row.update(calculate_stats(self.df[target], self.df[score], self.metric, self.metric_values, self.decimals))  
        ...
        if self.statistics_data is not None:
            self.statistics_data["Score"] = pd.Categorical(self.statistics_data["Score"], categories=self.score_columns, ordered=True)
            self.statistics_data["Target"] = pd.Categorical(self.statistics_data["Target"], categories=self.target_columns, ordered=True)
            self.statistics_data = self.statistics_data.sort_values(by=[self.top_level, self._get_second_level[self.top_level]])
            data = pd.merge(data, self.statistics_data, how='inner', on=["Score","Target"]) if data is not None else self.statistics_data
